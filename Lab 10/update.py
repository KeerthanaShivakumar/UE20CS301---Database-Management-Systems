...
def update():
    ...
    selected_result = get_train(selected_trains)
    if selected_result:
        ...
        if st.button("Update Train"):
            update_table(new_train_no, new_train_name, new_train_type, new_source, new_destination, new_availability, train_no, train_name, train_type, source, destination, availability)
            st.success("Successfully updated values for train:: {} to ::{}".format(train_name, new_train_name))

    result2 = view_all_trains()
